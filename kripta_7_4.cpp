...
public:
    ...
    Fraction(const cpp_int& n = 0, const cpp_int& d = 1) : num(n), den(d) {
        if (den == 0) throw std::invalid_argument("denominator zero");
...
public:
    ...
    static Fraction fromContinuedFraction(const std::vector<cpp_int>& cf) {
        if (cf.empty()) throw std::invalid_argument("empty cf");
        cpp_int num = 1, den = 0;
...
public:
    static std::string getPath(const Fraction& frac) {
        ...
        while (!(p == 1 && q == 1)) {
            if (p < q) { path.push_back('L'); q = q - p; }
            else { path.push_back('R'); p = p - q; }
        }
...
public:
    static std::string getPath(const Fraction& frac) {
        Pair left(0, 1), right(1, 0);
        std::string path;
        while (true) {
            Pair med(left.first + right.first, left.second + right.second);
            if (frac.num * med.second == med.first * frac.den) break;
    ...
    static Fraction fromPath(const std::string& path) {
        Pair left(0, 1), right(1, 0);
        for (char c : path) {
            if (c == 'L') right = Pair(left.first + right.first, left.second + right.second);
            else if (c == 'R') left = Pair(left.first + right.first, left.second + right.second);
            else throw std::invalid_argument("invalid char");
        }
...
public:
    RSAKeyPair generateFermaVulnerable(int bits = 1024) {
        ...
        for (int att = 0; att < 100; ++att) {
            ...
            if (d < utils::fourthRoot(n) / 3) continue;
            return { {n,e},{n,d,p,q} };
        }
        throw std::runtime_error("Ferma‑vulnerable key generation failed");
    }
    RSAKeyPair generateWienerVulnerable(int bits = 1024) {
        ...
        for (int att = 0; att < 100; ++att) {
            cpp_int p = utils::generatePrime(half);
            cpp_int q = utils::generatePrime(half);
            if (p > q) std::swap(p, q);
            cpp_int n = p * q;
            cpp_int diff = q - p;
            if (diff < utils::fourthRoot(n)) continue; 
            cpp_int phi = (p - 1) * (q - 1);
            cpp_int max_d = utils::fourthRoot(n) / 3;
            if (max_d < 2) continue;
            int d_bits = utils::bitLength(max_d);
            if (d_bits < 2) d_bits = 2;
            cpp_int d;
            do {
                d = utils::randomNumber(d_bits) % max_d + 2;
            } while (d < 2 || d >= max_d);
            cpp_int e;
            try {
        ...
        throw std::runtime_error("Wiener‑vulnerable key generation failed");
    }
};
class WienerAttack {
public:
    static std::pair<cpp_int, cpp_int> attack(const cpp_int& n, const cpp_int& e) {
        ...
        for (const Fraction& cand : conv) {
            cpp_int k = cand.num, d = cand.den;
            if (k <= 0 || d <= 0) continue;
...
void testContinuedFraction() {
    ...
    Fraction f(3, 4);
    auto cf = ContinuedFractionService::getContinuedFraction(f);
    assert(cf.size() == 3 && cf[0] == 0 && cf[1] == 1 && cf[2] == 3);
    assert(ContinuedFractionService::fromContinuedFraction(cf) == f);
    auto conv = ContinuedFractionService::getConvergents(f);
...
void testSternBrocot() {
    ...
    auto conv = SternBrocotTree::getConvergents("RL");
...
void testRSA() {
    ...
    auto keyF = rsa.generateFermaVulnerable(256);
    ...
    auto keyW = rsa.generateWienerVulnerable(256);
...
void testWienerAttack() {
    ...
    RSAService rsa;
    std::cout << "  Generating Wiener‑vulnerable key (256 bits)...\n";
    auto key = rsa.generateWienerVulnerable(256);
    std::cout << "  Running attack...\n";
    auto pq = WienerAttack::attack(key.public_key.n, key.public_key.e);
    ...
    auto key2 = rsa.generateFermaVulnerable(256);
    bool thrown = false;
    try { WienerAttack::attack(key2.public_key.n, key2.public_key.e); }
...
int main() {
    try {
        ...
        auto key = rsa.generateWienerVulnerable(1024);
        std::cout << "n = " << key.public_key.n << "\n";
        std::cout << "e = " << key.public_key.e << "\n";
        std::cout << "\nRunning Wiener attack...\n";
        auto pq = WienerAttack::attack(key.public_key.n, key.public_key.e);
        std::cout << "Recovered p = " << pq.first << "\n";
        std::cout << "Recovered q = " << pq.second << "\n";
        std::cout << "Check: p*q == n? " << (pq.first * pq.second == key.public_key.n ? "YES" : "NO") << "\n";
        std::cout << "\n=== Additional demonstration ===\n";
        Fraction demo(41, 13);
        std::cout << "Fraction " << demo << " -> continued fraction: [";
        auto cf = ContinuedFractionService::getContinuedFraction(demo);
        for (size_t i = 0; i < cf.size(); ++i) {
            if (i > 0) std::cout << ";";
            std::cout << cf[i];
        }
